...
func NewParserStatusManager(conf *configs.SearchServiceConfig, parsers ...search_interfaces.Parser) *ParserStatusManager {
	...
	for _, parser := range parsers {
		psm.parsersStats[parser.GetName()] = &search_interfaces.ParserStatus{
			Name:           parser.GetName(),
			LastCheck:      time.Now(),
			Initialized:    false,
			CircuitState:   "closed",
...
func (psm *ParserStatusManager) UpdateStatus(name string, success bool, err error) {
	...
	if !exists {
		status = &search_interfaces.ParserStatus{
			Name:        name,
			Initialized: true,
			LastCheck:   time.Now(),
		}
...
func (psm *ParserStatusManager) GetParserStatus(name string) (*search_interfaces.ParserStatus, bool) {
	// так как мэнеджер статуса парсеров основан на мапе, все панипуляции проводит под мьютексом
	psm.mu.RLock()
